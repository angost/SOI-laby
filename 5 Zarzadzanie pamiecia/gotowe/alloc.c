...
{
    register struct hole *curr_hole;
    /* initialising buffer of max size*/
    phys_clicks buffer[NR_HOLES * 2 + 1];
    phys_clicks buff_nr_of_elements, i;
    int buffer_bytes;

    i = 0;
    curr_hole = hole_head;
    buffer_bytes = mm_in.m1_i1;
    /* mm_in.m1_i1 - buffer size in bytes*/
    buff_nr_of_elements = buffer_bytes / sizeof(phys_clicks);

    while (curr_hole != NIL_HOLE && curr_hole->h_base < swap_base && (i + 2 < buff_nr_of_elements)) {
        buffer[i++] = curr_hole->h_len;
	      buffer[i++] = curr_hole->h_base;
        curr_hole = curr_hole->h_next;
    }
    buffer[i] = 0;
    /* sys_copy(src_proc, src_segm, src_addr, dst_proc, dest_segm, dest_addr, bytes);*/
    sys_copy(MM_PROC_NR, D, (phys_clicks) buffer, who, D, (phys_clicks) mm_in.m1_p1, (phys_clicks) buffer_bytes);
    return i/2;
 ...
 */
  ...
  register struct hole *hp, *max_hole, *prev_ptr;
  ...
  do {
      ...
      max_hole = hp;
      found = 0;
      /* Looking for fitting hole*/
      while (hp != NIL_HOLE && hp->h_base < swap_base) {
          /* Updating max_hole value (worst_fit)*/
          if (alloc_algorithm == 1 && hp->h_len > max_hole->h_len) {
              max_hole = hp;
          } /* (first_fit)*/
          else if (alloc_algorithm == 0 && hp->h_len >= clicks) {
              found = 1;
              break;
          }
          prev_ptr = hp;
      ...
      /* Checking if max_hole is big enough*/
      if (alloc_algorithm == 1 && max_hole->h_len >= clicks) {
          found = 1;
          hp = max_hole;
      }
      /* Using hole if it was found*/
      if (found) {
