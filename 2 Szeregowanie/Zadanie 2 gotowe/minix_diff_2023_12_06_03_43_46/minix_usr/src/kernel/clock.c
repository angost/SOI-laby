 ...
 */
  ...
  if (--sched_ticks == 0) {
    ...
    next_proc_g_nr = rdy_head[USER_Q]->group_nr;
    if (next_proc_g_nr < 0 || next_proc_g_nr > 2) {
      sched_ticks = SCHED_RATE;		/* reset quantum */
    } else {
      sched_ticks = SCHED_RATE * quants[next_proc_g_nr];

      /*if (next_proc_g_nr == 1) {
        printf(" = time %d | Time %d + %d tickow", rdy_head[USER_Q]->user_time, rdy_head[USER_Q]->user_time, sched_ticks);
      }*/

    }
    /*printf("Group: %d SumTicks: %d | ", next_proc_g_nr, sched_ticks);*/
