...
PRIVATE void print_queue() {
    struct proc *current_node;
    current_node = rdy_head[USER_Q];
    printf("\nQUEUEEE!!! ");
    while (current_node != NIL_PROC) {
/*        printf("%d ", current_node->nr);*/
        printf("Proc %s pid %d group %d | ", current_node->p_name, current_node->p_pid, current_node->group_nr);
        current_node = current_node->p_nextready;
    }
    printf("\n");
}

PRIVATE void change_order_simple_queue() {
...
PRIVATE void change_order_found_middle() {
    ...
    rdy_head[USER_Q]->p_nextready = rdy_tail[USER_Q]->p_nextready->p_nextready;
    /* Setting old head=new tail child to none */
    rdy_tail[USER_Q]->p_nextready->p_nextready = NIL_PROC;
    /* Setting old head as new tail */
    rdy_tail[USER_Q] = rdy_tail[USER_Q]->p_nextready;
}

PRIVATE void change_order_found_last() {
    /* Moving old head to the end */
    parent_node->p_nextready = rdy_head[USER_Q];
    /* New head takes over old head's children */
    rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q]->p_nextready;
    /* Setting old head=new tail child to none */
    rdy_head[USER_Q]->p_nextready = NIL_PROC;
    /* Updating head pointer */
    rdy_head[USER_Q] = rdy_tail[USER_Q];
    /* Updating tail pointer */
    rdy_tail[USER_Q] = parent_node->p_nextready;
}


PRIVATE void find_next_process() {
    struct proc *current_process;
    /* Maximum 3 tries */
    int try_nr;
    for (try_nr = 0; try_nr < 3; try_nr++ ) {
        ...
        current_process = rdy_head[USER_Q];
        while (current_process->p_nextready != NIL_PROC) {
            /* Found process from current group */
            if (current_process->p_nextready->group_nr == curr_group) {
 ...
 */
  ...
  curr_group = rdy_head[USER_Q]->group_nr;
  /*printf("HEAD: %s pid %d group %d \n", rdy_head[USER_Q]->p_name, rdy_head[USER_Q]->p_pid, rdy_head[USER_Q]->group_nr);
  print_queue();*/
  /*if (curr_group == 0) group_name = 'A';
  else if (curr_group == 1) group_name = 'B';
  else group_name = 'C';
  printf("gr %c pid %d -> ", group_name, rdy_head[USER_Q]->p_pid);*/

  /* 1 process in queue -> leave as it is */
  if (rdy_head[USER_Q] == rdy_tail[USER_Q]) {
    pick_proc(); return;
  }
  /* 2 processes in queue -> swap order */
  if (rdy_head[USER_Q]->p_nextready == rdy_tail[USER_Q]) {
      change_order_simple_queue(); pick_proc(); return;
  }

  /* 3+ processes in queue */
  find_next_process();
  if (parent_node == rdy_head[USER_Q]) {
      change_order_simple_queue();
  }
  else if (parent_node->p_nextready == rdy_tail[USER_Q]) {
