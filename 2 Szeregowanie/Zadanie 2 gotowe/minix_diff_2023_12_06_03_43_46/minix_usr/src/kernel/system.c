...
FORWARD _PROTOTYPE( int do_getTicks, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_GETTICKS:  r = do_getTicks(&m); break;
	    default:		r = E_BAD_FCN;
	}
...
register message *m_ptr;	/* pointer to request message */
{
    ...
    struct proc *p;
    int proc_index;
    int proc_pid = m_ptr->m1_i1;
    /* printf("Process pid %d", m_ptr->m1_i1);*/
    for(proc_index = 0; proc_index < NR_PROCS; proc_index++) {
        if (proc_addr(proc_index)->p_pid == proc_pid) {
          p = proc_addr(proc_index);
          m_ptr->m1_i1 = p->group_nr;
          /* printf(" group %d", m_ptr->m1_i1);*/
          return(OK);
        }
    }
    m_ptr->m1_i1 = -1;
...
{
    /* w m1_i1 - PID procesu */
    /* w m1_i2 - grupa jaka przypisac procesowi */
    struct proc *p;
    int proc_index;
    int proc_pid = m_ptr->m1_i1;
    /* printf("Process PID %d", m_ptr->m1_i1);*/
    for(proc_index = 0; proc_index < NR_PROCS; proc_index++) {
        if (proc_addr(proc_index)->p_pid == proc_pid) {
          p = proc_addr(proc_index);
          p->group_nr = m_ptr->m1_i2;
          /* printf(" group %d", p->group_nr);*/
          return(OK);
        }
    }
    return(OK);
...
PRIVATE int do_getQuants(m_ptr)
register message *m_ptr;	/* pointer to request message */
{
    /* w m1_i1 - indeks (numer) grupy */
    /* printf("Group %d ", m_ptr->m1_i1); */
...
PRIVATE int do_setQuants(m_ptr)
register message *m_ptr;	/* pointer to request message */
{
    /* w m1_i1 - indeks (numer) grupy
       w m1_i2 - nowa liczba kwantow dla grupy */
    /* printf("Group %d", m_ptr->m1_i1);*/
    quants[m_ptr->m1_i1] = m_ptr->m1_i2;
    /* printf(" new quants %d=%d", m_ptr->m1_i2, quants[m_ptr->m1_i1]);*/
    return(OK);
}

PRIVATE int do_getTicks(m_ptr)
register message *m_ptr;	/* pointer to request message */
{
    /* w m1_i1 - PID procesu */
    struct proc *p;
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->child_utime = 0;
  rpc->child_stime = 0;
  rpc->group_nr = 0; /*Setting default group nr*/
