...
PUBLIC int do_getgroup(){
	int g_nr = _taskcall(SYSTASK, SYS_GETGROUP, &mm_in);
	mp->mp_reply = mm_in;
	return g_nr;
}
...
PUBLIC int do_getquants(){
	int quants = _taskcall(SYSTASK, SYS_GETQUANTS, &mm_in);
	mp->mp_reply = mm_in;
	return quants;
}
...
PUBLIC int do_getticks(){
	int ticks = _taskcall(SYSTASK, SYS_GETTICKS, &mm_in);
	mp->mp_reply = mm_in;
	return ticks;
}
